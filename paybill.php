...
if ($person->checkDepositStat() == true){
    $person->feedback("done","您已支付完成，请点击下方支付完成按钮");
    exit();
}
...
$payAmount = 200;          //付款金额，单位:元
$orderName = 'NFLS-Prom2020-'. $person->getName() .'-订金';    //订单标题
...
if($result['code'] && $result['code']=='10000'){
    //生成商户端订单
	$tstamp = time();
	$person->dbInsert("alipay_order_id",["create_time"=>"$tstamp", "subject"=>"$orderName", "out_trade_no"=>"$outTradeNo", "amount"=>"$payAmount", "user_login_id"=>"$loginID", "trade_status"=>'0', "trade_type"=>'0', "cancel_flag"=>'0']);
    //生成二维码
